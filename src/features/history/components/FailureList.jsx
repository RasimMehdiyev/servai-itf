  ...
  return (
    <Card>
      <div className="gap-row" style={{ marginBottom: 12 }}>
        ...
        <h2 style={{ fontSize: 'clamp(15px,2vw,17px)', fontWeight: 600 }}>Top Failures</h2>
      </div>
        ...
        {failures.map(f => (
          <div key={f.id}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <strong style={{ fontSize: 'clamp(13px,1.6vw,15px)' }}>{f.title}</strong>
              <span className="chip chip--danger" style={{ fontSize: 10, padding: '2px 7px' }}>{f.count}×</span>
