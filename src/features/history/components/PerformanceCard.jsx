  ...
  return (
    <Card>
      <div className="gap-row" style={{ marginBottom: 12 }}>
        ...
        <h2 style={{ fontSize: 'clamp(15px,2vw,17px)', fontWeight: 600 }}>Performance</h2>
      </div>

      <div style={{ textAlign: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 700, color: 'var(--success)' }}>{successRate}%</div>
        ...
        {bars.map((bar, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
              ...
              />
              <div className="text-sm" style={{ marginTop: 4, color: 'var(--text-tertiary)', fontSize: 'clamp(10px,1.2vw,11px)' }}>{bar.label}</div>
            </div>
          )
        })}
      </div>
